// taken from trajopt: https://github.com/joschu/trajopt
...
#include "osgviewer/openrave_userdata_utils.hpp"
...
osg::Node* osgNodeFromGeom(const KinBody::Link::Geometry& geom) {
  ...
  default:
    ...
    printf("don't know how to make an osg node for geometry of type\n");
    break;
  }
  ...
  if (!geom.GetRenderFilename().empty()) {
    ...
    if (node) {
      ...
      printf("failed to load graphics mesh\n");
      return geode;
    }
  }
  else {
...
void OSGViewer::RemoveKinBody(OpenRAVE::KinBodyPtr body) {
  ...
  if (node) {
    ...
    printf("tried to remove kinbody that does not exist in osg\n");
  }
}
...
void OSGViewer::AddKeyCallback(int key, const KeyCallback& cb, const std::string& help) {
  ...
  if (it==m_handler->key_cbs.end()) {
    ...
    //LOG_ERROR("error: key %c was already bound!", key);
    printf("error: key was already bound!\n");
  }
...
void OSGViewer::SetTransparency(OpenRAVE::KinBodyPtr body, float alpha) {
  ...
  if (node) {
    ...
    printf("SetTransparency: body doesn't exist in osg!\n");
  }
}
...
GraphHandlePtr OSGViewer::PlotLink(KinBody::LinkPtr link) {
  ...
  if (it == orig->links.end()) {
    ...
    printf("want to plot link, can't find it");
...
OpenRAVE::GraphHandlePtr OSGViewer::drawtrimesh (const float *ppoints, int stride, const int *pIndices, int numTriangles, const boost::multi_array< float, 2 > &colors) {
  osg::DrawElementsUInt* deui = new osg::DrawElementsUInt(GL_TRIANGLES);
  osg::Vec3Array* vec = new osg::Vec3Array();
  osg::Vec3Array& points = *vec;
  ...
  for(size_t i = 0; i < colors.shape()[0]; ++i) {
    osgcolors->push_back( osg::Vec4(colors[i][0], colors[i][1], colors[i][2], colors[i][3]) );
  }

  osg::Geometry* geom = new osg::Geometry;
  geom->setVertexArray( vec );
  geom->setColorArray( osgcolors );
  geom->setColorBinding( osgcolors->size() == points.size() ? osg::Geometry::BIND_PER_VERTEX : osg::Geometry::BIND_OVERALL );
...
/*
  ...
  state->setAttribute(mat);
*/
  if (colors.shape()[1] == 4 && colors[0][3] < 1) {
    state->setAttributeAndModes(new osg::BlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA) );
    state->setRenderingHint(osg::StateSet::TRANSPARENT_BIN );
  } else {
